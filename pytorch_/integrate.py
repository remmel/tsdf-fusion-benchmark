...
class TSDFVolumePytorch(TSDFVolumeBase):
  using = "Using vanilla PyTorch."
  useGPU = None # Handle both GPU and CPU, set later
  ...
  def integrate(self, color_im, depth_im, cam_intr, cam_pose, obs_weight):
    ...
    return self.integrate_static(color_im, depth_im, cam_intr, cam_pose, obs_weight,
                            self._world_c,
                            self._vox_coords,
                            self._weight_vol,
                            self._tsdf_vol,
